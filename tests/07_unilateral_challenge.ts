...
import { setup, ensureConfig, newSession, sleep, TEST_CHALLENGE_WINDOW, MPL_CORE } from "./helpers";
...
describe("07 unilateral close + challenge (M6)", () => {
  ...
  it("challenge after the deadline is rejected", async () => {
    const { provider, consumer, session } = await newSession(sdk, connection, { deposit: 1 });
    const cosign = (st: StateUpdate) => ({ providerSig: signState(provider.secretKey, st), consumerSig: signState(consumer.secretKey, st) });
    const s1: StateUpdate = { session, nonce: 1n, owedToProvider: 100_000_000n, unitsConsumed: 1n, timestamp: 1n };
    await sdk.initiateUnilateralClose({ caller: provider, provider: provider.publicKey, consumer: consumer.publicKey, session, state: s1, ...cosign(s1) });
    await sleep((TEST_CHALLENGE_WINDOW + 2) * 1000);
    const s2: StateUpdate = { session, nonce: 2n, owedToProvider: 200_000_000n, unitsConsumed: 2n, timestamp: 2n };
    try {
      await sdk.challenge({ caller: consumer, provider: provider.publicKey, consumer: consumer.publicKey, session, state: s2, ...cosign(s2) });
