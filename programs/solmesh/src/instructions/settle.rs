...
pub fn settle_handler(ctx: Context<Settle>, state: StateUpdate) -> Result<()> {
    apply_state(&ctx, &state, false)
}

/// Mid-session checkpoint settlement (does not close). `is_checkpoint=true`.
pub fn checkpoint_handler(ctx: Context<Settle>, state: StateUpdate) -> Result<()> {
    apply_state(&ctx, &state, true)
}

fn apply_state(ctx: &Context<Settle>, state: &StateUpdate, is_checkpoint: bool) -> Result<()> {
    {
    ...
    // Pay provider + fee from the escrow PDA.
    ...
    // Update reputation on the Core NFT.
